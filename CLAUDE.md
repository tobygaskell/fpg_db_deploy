...
- The `LOGS` and `CHOICES` copies in `populate_dev.sql` name their columns. `populate_dev.sql` copies `FPG.*` into the current schema, and `DEV_FPG` is ahead of `FPG` for as long as a migration is being rehearsed, which is exactly when the script gets run. A `SELECT *` copy of a table whose shape has changed fails with "Column count doesn't match value count", after the truncate, leaving the table empty; the migration is then marked unapplied, so `yoyo apply` fixes it, not `reapply`, which is a silent no-op on an unapplied migration. `CHOICES` was named for this reason when `FIXTURE_ID` was dropped and `LOGS` when `RUN_ID` was added; both windows have since closed, but the next migration opens another.
- `populate_dev.sql` is not in the repo: it is gitignored beside `yoyo.ini` and exists only on the laptop and as an applied row in `DEV_FPG._yoyo_migration`. It truncates seventeen tables (`USERS`, `TOKENS` and the four mini-league tables among them) and copies `FPG.*` into the current schema inside a `SET FOREIGN_KEY_CHECKS = 0` / `= 1` pair, because a parent of a foreign key cannot be truncated otherwise, then deletes rows in the session tables it does not copy (`REFRESH_TOKENS`, `MCP_TOKENS`, the OAuth tables) whose account is not in the copied `USERS`, so the keys to `USERS` hold; the `CHOICES` and `LOGS` copies name their columns, for the reason above. It is otherwise column-order dependent and would wipe production if ever applied to `FPG`; only reapply it against `DEV_FPG`.
